...
export class BarracksPanelScene extends UiScene {
  ...
  private buildFilledSlot(hero: Hero, x: number, y: number, slotBgH: number): void {
    // Selection highlight: gold border when this hero is selected.
    // No 'selected' button style yet (Cluster B · 48); we use a raw Phaser
    // rectangle with manual stroke to achieve the visible highlight.
    ...
    const card = new PixuiHeroCard(this, hero, { size: 'small' });
    // pixui Containers are positioned relative to their pixui parent's anchor, not world coords.
    // Wrapping in a topLeft.frame with absolute canvas coords lets us place each card at a
    // specific (x, y) without recomputing slot-frame-relative offsets per slot.
