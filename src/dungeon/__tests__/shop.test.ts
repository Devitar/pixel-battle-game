...
describe('generateShop', () => {
  it('returns 4 items', () => {
    const { inventory } = generateShop(1, 1, createRng(1));
  ...
  it('prices are positive integers', () => {
    const { inventory } = generateShop(2, 1, createRng(1));
  ...
  it('floor-1 common-rarity item: price in [25, 35]', () => {
    for (let seed = 1; seed < 50; seed++) {
      const { inventory } = generateShop(1, 1, createRng(seed));
      for (const slot of inventory) {
  ...
  it('floor-3 rare-rarity item: price in [510, 690] (when found)', () => {
    ...
    for (let seed = 1; seed < 200 && !foundRare; seed++) {
      const { inventory } = generateShop(3, 1, createRng(seed));
      for (const slot of inventory) {
  ...
  it('determinism: same seed produces identical inventory', () => {
    const a = generateShop(1, 1, createRng(42));
    const b = generateShop(1, 1, createRng(42));
    expect(a).toEqual(b);
  });

  it('floor 3 average price is roughly 2-4.5x floor 1 average', () => {
    const { inventory: f1 } = generateShop(1, 1, createRng(99));
    const { inventory: f3 } = generateShop(3, 1, createRng(99));
