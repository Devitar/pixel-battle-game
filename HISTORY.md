...
- **Decisions** (Q1–Q5 in spec):
  - **Q1 — Single spec for all 6 panels + cleanup.** Mirrors 3b's "4 panels in one spec" precedent; foundation work (3c-i) had already de-risked the unknowns.
  - **Q2 — Don't extract `pixuiItemIcon()` or `pixuiSlotSquare()` helpers.** Three icon sites with different shapes; helper signatures would be fiddly for ~30 lines saved. Per CLAUDE.md "Three similar lines is better than a premature abstraction." Equip's slot strip and Barracks's mini-equip-strip stayed inline; verdict held during impl.
  - **Q3 — `pixui.Button` for perk cards initially, fallback B (Frame + Clickable + manual border swap) when needed.** Single-string `ButtonConfig` can't render title + description hierarchy; PerkOverlay and EventOverlay's choice rows used fallback B. Single-line buttons (camp_screen actions, page arrows, commit, retire/cancel/confirm) used `pixui.Button` directly.
  - **Q4 — Smallest → largest, mechanical first.** Perk → Event → camp_screen → Barracks → Expeditions → Equip → cleanup. Pattern settled before complex consumers stress-tested the foundation.
  - **Q5 — Close Cluster B · 51 inline.** README rewrite folded into 3c-ii Task 9 (asset layout + pixel-tools pipeline + Windows shim sections; vite/assets.mjs field comments; src/README.md stale-reference cleanup). Single coherent post-migration README.
  - **Sub — Include `camp_screen_scene` in scope.** Not in the original "11 panels" count but a `HeroCard` consumer; without it, `hero_card.ts` couldn't fully retire.
- **Surprises:**
  - **`events.on(...)` for RESUME stacks across `scene.restart()`.** Phaser's Scene.events EventEmitter doesn't auto-clean listeners on restart. Sibling `shop_overlay_scene.ts:89` already used the safe `events.once(...)` pattern; Tasks 3 (camp_screen) and 4 (Barracks) initially used `events.on(...)` and were caught in review. Fix: switch to `events.once(...)` everywhere — listener fires once, removes itself, fresh `create()` registers a new once-listener. Tasks 5 + 6 had no resume listeners.
  - **Expeditions slot-to-slot drag was a behavioral regression in Task 5's first cut.** Original Phaser implementation made every eligible hero's card draggable; placed heroes in slots could be dragged to other slots (formation rearrangement). First migration only made eligible-grid cards draggable, leaving the slot-swap branch as dead code. Fix: extract `installDragHandlers(card, hero, frameAnchorX, frameAnchorY)` helper; call for both slot cards AND eligible-grid cards. All four drag cases now work (grid→empty, grid→filled-displace, slot→empty, slot→filled-swap).
  - **Equip's first migration was too shallow.** 1 pixui `insert.X.Y(...)` call vs ~50+ raw `this.add.X(...)`. Implementer reasoned that "click → restart" interaction model didn't benefit from `pixui.Button`'s managed state. Spec compliance review correctly flagged this as a deviation from "all panels on pixui" — visual-clash with sibling pixui panels. Re-migration achieved Barracks-parity ratio (14 pixui insert calls + ~26 raw Phaser at spec-sanctioned sites: dim overlay, dynamic-stroke selection borders, rarity-bordered slot squares, sprite icons, multi-instance colored stat/kit tokens, slot-detail card with rarity-tinted before/after summaries).
  - **Equip page-cursor overflow on commit.** When `commit()` removed the last item on a page, `_pickerPageStart` could exceed `items.length`, leaving the picker empty but page arrows still functional. Fix: clamp `_pickerPageStart` after equipFromStash/Pack/unequipToStash/Pack mutations.
  - **EventOverlay close path: spec said `'dungeon'`, original code used `'corridor'`.** Implementer preserved the actual original behavior; spec doc was wrong. Same precedent as the 3c-i HISTORY note about preserving actual behavior over plan-doc errors.
  - **Plan's "extend `dev/main_scene.ts` for verification" approach didn't generalize from 3c-i.** Confirmed in 3c-ii brainstorm: dev scene migration to UiScene is disproportionate work. Manual browser smoke against the migrated panels themselves is the verification path; user drives it.
- **Source:** spec `docs/superpowers/specs/2026-05-08-pixui-hero-card-panels-design.md`; plan `docs/superpowers/plans/2026-05-08-pixui-hero-card-panels.md`. Test count: 1730 → 1716 (14 `panel_layout.test.ts` tests deleted alongside the retired helper). Cluster B · 45 closes (entire pixui adoption initiative); Cluster B · 51 closes (README docs).
...
- **Decisions:**
  ...
  - **`CombatActor` for heroes throughout travel + combat.** Single rendering path — same actor, same HP bar, same name label. Travel HP tick and combat damage write to the same `setHpBar` call. Bob applies to actor container; rotate to inner paperdoll only via new `bodyVisual` getter (so HP bar + name don't tilt; getter named to dodge Phaser's reserved `body` arcade-physics property).
  - **Combat scene file deleted.** Its responsibilities (background, HUD, actor build, playback wiring, handoff) absorbed into corridor scene. Combat handoff module deleted too. End state: one large corridor scene file (~860 lines); dungeon scene narrows to map-only (~330 lines, down from 851).
  - **First-node entry uses an explicit "Click to Engage" prompt in corridor instead of a walk-in tween.** Phase 4 `awaitingEngage` gate moves to corridor as a pulsing prompt overlay. Click target is a generous hit rectangle on the prompt itself, not scene-wide — scene-wide listener would have fired alongside the FF speed toggle and accidentally engaged when the player clicked 1×/3×.
  - **Result/wipe panels render in corridor.** Combat ends in-place; panel slides over the corridor backdrop. After dismiss, transition to dungeon scene only when the player needs the map (fork pick after combat).
  - **Walk-speed and combat-speed kept as separate Preferences.** Two FF rectangles at the same screen slot, only one visible at a time (walk-speed during travel, combat-speed during combat). The `toggleWalkSpeed` handler routes to `toggleCombatSpeed` when combat HUD is active so the F key works in both modes without re-binding.
- **Surprises:**
  - **Phaser's `body` property is reserved for arcade physics.** Initial `CombatActor.body` accessor for the bob/rotate split caused a typecheck error against Phaser's `Body | StaticBody | BodyType | null`. Renamed to `bodyVisual`. Worth knowing for any future `CombatActor` accessor.
  - **Combat-speed FF toggle was missed in the first Task 5 implementation pass.** The deleted combat scene had a FF button calling `playback.setSpeed`; the corridor scene only read `combatSpeed` once at combat start. Spec reviewer caught it; fix-up added the second FF UI (toggle + hide/restore lifecycle) before Task 5 was accepted.
  - **Hero coordinate system needed careful alignment.** Combat scene's `ROW_Y=300` puts hero feet at y≈324 (via the actor's internal `FLOOR_Y=24` + body half). Travel scene's old `GROUND_Y=380` was a different coordinate concept. Reconciling required moving the corridor's floor line to y=324 — heroes appear higher up, with more empty space below for combat HUD.
  - **Enemy entrance via scrolling world** is described in spec §2.4 as the intended behavior, but Task 5 ships with the simpler "spawn at scroll completion" approach. World-embedded enemy entrance is a polish improvement deferred to a follow-up; small gap between spec intent and shipped behavior.
...
- **Decisions:**
  - **Three visibility layers, not two.** Original Q2 had revealed/fogged; user feedback during impl review surfaced that a fully-fogged road-not-taken erased useful information ("which fork branch did I pass up?"). Added a `ghost` layer: nodes in past rows the party did NOT walk through render at low opacity (`GHOST_ALPHA = 0.25`) instead of disappearing. Preserves the cartographer-log story without flattening the contrast of the chosen path.
  - **Click-to-advance after every node, not just forks.** Smoke-test surfaced two bugs at once: (a) auto-advancing combat→combat felt jarring; (b) shops/camps/treasures sitting at fork-source positions silently picked branch 0 because their leave-functions auto-advanced. The single fix: every node-clear function (`completeCombat`, `leaveShop`, `claimTreasure`, `chooseCampNodeEffect` non-leave, and the event overlay's post-applyChoice handoff) now sets `awaitingFork=true` and leaves `currentNodeId` put. The scene's existing fork-pick logic naturally lights up the next-row choices as click targets — works whether there's one or many. The flag's name is now a slight misnomer (it covers any next-choice click, even a single one); kept as-is to limit blast radius. The dungeon scene's RESUME handler skips its no-op tween when awaitingFork is set so overlay-close transitions are snappy.
  - **Explicit cartographer log over backward-reachability.** Phase 1's `isNodeCleared` walked predecessor edges from `currentNodeId`; for richer DAGs it would mark BOTH branches at a past converging fork as cleared, but only one was actually walked. New `traversedNodeIds: readonly string[]` field on `RunState` records the actual path. Phase 1 explicitly flagged this as Phase-2-or-later work; Phase 3 is when it became load-bearing (an unrevealed past branch needs to be distinguishable from a revealed one — and now also from a ghost).
  - **Visibility helper as a separate module.** `src/dungeon/visibility.ts` mirrors the `map_layout.ts` split — pure-TS, unit-tested, scene consumes the result. The BFS-depth + lookahead + ghost-membership computation is testable without Phaser; 15 unit tests cover the fog edge cases (boss-discover row, ghost membership, ghost-edge predicate).
  - **Save schema stays at 1.** `traversedNodeIds` defaults to `[currentNodeId]` for old in-progress saves via `normalizeSaveFile`. Old saves lose their prior cartographer trail (one-time cost on upgrade); the field works correctly going forward and no migration is needed. Mirrors the `lost ?? []` precedent.
  - **`playerPath` left untouched.** No production callers; only its own three tests reference it. Removing it is unrelated churn — defer to a future cleanup.
- **Surprises:**
  - **`refreshEdges` had to move inside `refreshNodeStates`.** Edge visibility is derived from the visibility set, which itself is recomputed on every state change. The original Phase 1 code called `refreshEdges` once at scene setup; that was fine when edges were static, but fog slides forward as the party advances. Coupling the two refreshes was a one-line move; worth knowing for anyone touching the scene later.
  - **The "boss is fogged at start" property is a single-test guard against an off-by-one in the lookahead window.** Floor 1 has 8 rows, boss at row 7, lookahead 2; visibility from row 0 covers rows [0, 2]. If lookahead were inclusive in a way that also captured row 7, the locked design would silently break. Worth keeping as a regression sentinel — alongside its sibling test "future-row nodes are fogged, not ghosted" which guards the ghost-layer formulation against the same off-by-one (ghost is past-only, never future).
- **Source:** TODO.md Cluster B · 30 Phase 3. Plan: `docs/superpowers/plans/2026-05-03-map-dungeon-phase-3-fog.md`. Spec is the locked Q2 design embedded in the TODO entry plus the road-not-taken refinement raised during execution review (mirrors Phase 1's "spec lives in TODO" convention). Test count delta: 1445 → 1469 (+24: 7 traversed-lifecycle, 2 save normalizer, 15 visibility module).
