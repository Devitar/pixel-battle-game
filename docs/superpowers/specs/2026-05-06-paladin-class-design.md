...
| sub | `swapTarget` / `weaponSwaps` | None. Paladin is sword-or-bust thematically; axe/dagger Paladin reads weird. Without a `swapTarget`, off-preferred melee weapons leave the kit unchanged (Archer pattern). No new swap-variant abilities to author. |
...
- `consecrate` 3 HP/turn × 3 turns × 3 allies = 27 total party HP per cast on cd 4. Roughly equivalent to one Lay on Hands spread across the party. **Fixed-amplitude (does NOT scale on Mind)** — the existing `poison` effect kind has no `scalingStat` field. Adding mind-scaling to Consecrate would require engine changes outside this spec's scope. Acceptable: party HoT shape is the value, not the per-tick number.
